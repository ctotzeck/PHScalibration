''' Code written by C. Totzeck, Dec 2022
    ...
    Run the code to obtain the data and results of the second test case presented in the article
...
noise_flag = True #True # True/False
sigma = 0.25
...
u[:,0] = 10.*np.sin(2.*np.pi/T*np.linspace(0,T,steps+1))
u[:,1] =  5.*np.cos(4.*np.pi/T*np.linspace(0,T,steps+1))
...
if noise_flag:
    ydata = ref_y + sigma*np.random.randn(helpA,helpB)
else:
...
print('norm B',np.sum(abs(BT-B)))
print('norm Q',np.sum(abs(QT-Q)))
print('norm J',np.sum(abs(JT-J)))
print('norm R',np.sum(abs(RT-R)))
print('norm x',np.sum(abs(xT-x0)))
...
while (cn/costStart*100 > 0.01) and i<maxGradSteps:
...
if alternative:
    ...
    np.save('cost_noise{0}.npy'.format(sigma), cost_vec)
    np.save('J_noise{0}.npy'.format(sigma), norm_vec_J)
    np.save('Q_noise{0}.npy'.format(sigma), norm_vec_Q)
    np.save('R_noise{0}.npy'.format(sigma), norm_vec_R)
#     np.save('B_noise{0}.npy'.format(sigma), norm_vec_B)
...
# plt.savefig('cost_noise{0}.png'.format(sigma))

plt.figure(2)
plt.plot(time_vec,yn)
plt.xlabel('t')
plt.ylabel(r'$y(t)$')
plt.savefig('modeloutput_noise{0}.png'.format(sigma))

plt.figure(3)
plt.plot(time_vec, ydata)
plt.xlabel('t')
plt.ylabel(r'$y_\mathrm{data}(t)$')
plt.savefig('dataoutput_noise{0}.png'.format(sigma))

plt.figure(5)
plt.plot(time_vec, yn-ydata)
plt.xlabel('t')
plt.ylabel(r'$y(t) -y_\mathrm{data}(t)$')
plt.savefig('diffoutput_noise{0}.png'.format(sigma))
...
plt.savefig('diffnorms_noise{0}.png'.format(sigma))


# plot for comparison of cost
c001=np.load('cost_noise0.01.npy')
c005=np.load('cost_noise0.05.npy')
c025=np.load('cost_noise0.25.npy')
...
plt.semilogy(c005)
plt.semilogy(c025)
plt.legend([r'$\sigma=0.01$',r'$\sigma=0.05$', r'$\sigma=0.25$'])
plt.xlabel('iteration')
plt.ylabel(r'$\hat\mathcal J$')
plt.savefig('cost_noise_all.png')
#
# plt.show()
#
# plot for comparison of matrices
Q001=np.load('Q_noise0.01.npy')
Q005=np.load('Q_noise0.05.npy')
Q025=np.load('Q_noise0.25.npy')
R001=np.load('R_noise0.01.npy')
R005=np.load('R_noise0.05.npy')
R025=np.load('R_noise0.25.npy')
J001=np.load('J_noise0.01.npy')
J005=np.load('J_noise0.05.npy')
J025=np.load('J_noise0.25.npy')
B001=np.load('B_noise0.01.npy')
B005=np.load('B_noise0.05.npy')
B025=np.load('B_noise0.25.npy')


plt.figure(7)
cmap = plt.get_cmap("tab10")
# #plt.semilogy(cost_vec)
plt.plot(Q001,color=cmap(0))
plt.plot(Q005,color=cmap(0), linestyle='--')
plt.plot(Q025,color=cmap(0), linestyle=':')
plt.plot(R001,color=cmap(1))
plt.plot(J001,color=cmap(2))
plt.plot(B001,color=cmap(3))
plt.plot(Q005,color=cmap(0), linestyle='--')
plt.plot(R005,color=cmap(1), linestyle='--')
plt.plot(J005,color=cmap(2), linestyle='--')
plt.plot(B005,color=cmap(3), linestyle='--')
plt.plot(Q025,color=cmap(0), linestyle=':')
plt.plot(R025,color=cmap(1), linestyle=':')
plt.plot(J025,color=cmap(2), linestyle=':')
plt.plot(B025,color=cmap(3), linestyle=':')
plt.legend([r'$\sigma=0.01$',r'$\sigma=0.05$', r'$\sigma=0.25$'])
plt.xlabel('iteration')
plt.ylabel('Frobenius norm')
plt.savefig('Fnorm_noise_all.png')
