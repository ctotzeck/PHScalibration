...
noise_flag = False #False # True/False

maxGradSteps = 100

# set true values to be identified later
BT = np.load("B.npy") #init_B() #np.dot(A, A.transpose())
QT = np.load("Q.npy") #np.np.eye(N)
JT = np.load("J.npy") #generate_skew_symm(N)
RT = np.load("R.npy") # 1./N*np.diag(range(1,N+1)) #np.dot(A, A.transpose())
...
alternative = False #alternative retraction computation Q
...
u[:,1] =  5.*np.cos(4.*np.pi*np.linspace(0,T,steps+1)) #np.ones([steps+1,M]) + np.random.rand(steps+1,M) #np.ones([steps+1,M]) #1./M*np.random.rand(steps+1,M)
...
if noise_flag:
    ydata = ref_y + 0.0005*np.random.randn(helpA,helpB)
else:
...
plt.savefig('initialoutput_MSD.png')


costStart = evaluate_cost(yn,ydata)
pvec = ref_y[::100]

cn = 0.0 # just a dummy value to start loop
...
while ((costStart-cn)/costStart*100) > 0.01 and i<maxGradSteps:
