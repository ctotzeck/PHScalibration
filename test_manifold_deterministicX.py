...
noise_flag, M, N, maxGradSteps, T, steps, dt, lam = initialilze_problem_parameters_X()
alternative, gamma, sig, fail_flag, eps, sol_x, ref_x, t_vec = initialize_solver_parameters_X(steps,N,T)
cost_vec, norm_vec_Q, norm_vec_J, norm_vec_R, norm_vec_B = initialize_container_postprocessing()

u = initialize_input_X(M,T,t_vec,steps)
BT, QT, JT, RT, xT = initialize_reference_data_X(N,M)
...
B, Q, J, R = BT, QT, JT, RT
x0 = initialize_for_optimization_X(N,M)
...
u_rob = input_robustness_test_X(t_vec,T,steps,M)
x_rob_init,yn_rob_init = solve_forward(x0,J,R,Q,B,u_rob,steps,N,dt)

plot_solution_initial_setting_X(t_vec,yn)
...
while (cn > 0.0003*cost_vec[0]) and i<maxGradSteps:
    ...
    JA, RA, QA, BA, x0A, xA, yA, fail, cA = armijo_rule_X(xn,yn,J,R,Q,B, x0, dir_J,dir_R, dir_Q, dir_B, dir_x,ydata,u,sig,gamma,fail_flag,eps,alternative,dt,steps,N,lam)
...
plots_postprocessing_X(cost_vec,t_vec,yn,ydata,norm_vec_B,norm_vec_Q,norm_vec_J,norm_vec_R,i)
print('----- robustness test ------')
u = input_robustness_test_X(t_vec,T,steps,M)
Rtest_ref_x, Rtest_ref_y, Rtest_x, Rtest_y = run_robustness_test_X(xT,JT,RT,QT,BT,u,x0,J,R,Q,B,steps,N,dt)
plot_robustness_test_X(Rtest_ref_x, Rtest_ref_y, Rtest_x, Rtest_y, t_vec, dt)
# show plots
plt.show()